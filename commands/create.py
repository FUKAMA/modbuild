...
def Execute(args):
    ...
    # template_url = "https://github.com/FUKAMA/TempModule/archive/refs/heads/main.zip"
    # template_zip = "TempMod.zip"
    # subprocess.run(["wget", template_url, "-O", template_zip], check=True,shell=True)
    ...
    # # モジュールのテンプレートをダウンロード
    # subprocess.run(
    #     ["wget","https://github.com/FUKAMA/TempModule/archive/refs/heads/main.zip","OutFile","TempMod.zip"]
    #     shell=True
    # )        
    # # ダウンロードしたファイルを解凍
    # subprocess.run(["tar","-xf","TempMod.zip"])
    # # 解凍したファイルを移動
    # subprocess.run(
    #     ["Get-ChildItem","-Path","./TempMpdule-main","-Recurse","|","Move-Item","-Destination","."]
    #     shell=True
    # )

    # # 展開に使ったディレクトリをファイルを削除
    # subprocess.run(["rmdir","TempModule-main"])
    # subprocess.run(["del","TempMod.zip"])
